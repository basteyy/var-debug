...
<head>
    ...
    <style>
        ...
        .accordion input:not(:checked) ~ .body {
            height: 0;
            display: none;
        }

        .accordion input:checked ~ .body {
            height: auto;
            display: block;
        }
        ...
        .btn_group button.copy:hover {
            cursor: copy;
        }

        .btn_group button.search:hover {
            cursor: pointer;
        }
...
<script>
    ...
    /** Add a copy and search button to every code box **/
    document.querySelectorAll("code").forEach(function (element, index, array) {
        ...
        /** add button to the btn group **/
        btn_group.appendChild(copy);

        /** search button **/
        let search = document.createElement("button");
        search.classList.add("search");
        search.setAttribute("data-debug", element.getAttribute("id"));
        search.innerHTML = "❓";

        /** attach click event to button **/
        search.addEventListener("click", function () {
            let text = element.innerText || element.textContent;
            let search = prompt("Search for:", text);
            if (search !== null) {
                window.open("https://www.google.com/search?q=" + encodeURI("+php " + text), "_blank");
            }
        });

        /** add button to the dom **/
        btn_group.appendChild(search);


        /** add btn group to the dom **/
        element.parentNode.insertBefore(btn_group, element.nextSibling);

    });

    /** Double-click in code area selects all text **/
    document.querySelectorAll("code").forEach(function (element, index, array) {
 ...
 */
    ...
    }})();hljs.registerLanguage("php",e)})();/*! `xml` grammar compiled for Highlight.js 11.9.0 */
    (()=>{var e=(()=>{"use strict";return e=>{
